...
class PaymentController {
    constructor() {
        this.createOrder = async (req, res) => {
            try {
                const [token, url] = await (0, paypal_1.createOrder)(String(req.body.amount));
        ...
        this.captureorder = async (req, res) => {
            try {
        ...
        this.gethallcodes = async (req, res) => {
            try {
                ...
                res.status(200).json({ 'status': 'success', 'codes': codes.map((e) => { return e.code; }) });
                return;
            }
            catch (err) {
                res.status(400);
