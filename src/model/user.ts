...
export class User {
  ...
  async create(u: user): Promise<user> {
    try {
      const sql =
        'INSERT INTO users (email,password,name,phone,city,role) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *';
      // @ts-ignore
      const conn = await pool.connect();
  ...
  async checkResetCode(email: string, resetCode: string): Promise<string> {
    try {
      const sql_valid =
        'SELECT from users where email=$1 and password_verified_code=$2';
      const sql_expire =
        'SELECT from users where email=$1 and password_verified_code=$2 and password_reset_expires > Now()';
      // @ts-ignore
      const conn = await pool.connect();
